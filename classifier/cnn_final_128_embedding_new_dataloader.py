...
def setupClassifier(training_input_directory, validation_input_directory, testing_input_directory, classCount,
                    spec_count, segment_length, learning_rate, output_path):
    sys.stdout = c1
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(len(args.train_list)):
            training_set_file_list.append(
                args.train_test_split_path + '/b_' + str(args.train_list[i]) + '_training.list')
            training_files = read_file(training_set_file_list[i])
            feat_dim = int(args.train_list[i])
            if len(training_files) == 0:
                continue
            for batch in dataloader(training_input_directory, training_files, args.sgd_batch_size, feat_dim,
                                    shuffle=True, last_batch=True):
        ...
        print(
            "{} Training done in {} seconds. Training Loss {}".format(epoch, time.time() - start_time, epo_train_loss))
        ...
        for i in range(len(args.val_test_list)):
            validation_set_file_list.append(
                args.train_test_split_path + '/b_' + str(args.val_test_list[i]) + '_validation.list')
            validation_files = read_file(validation_set_file_list[i])
            feat_dim = int(args.val_test_list[i])
            if len(validation_files) == 0:
                continue
            for batch in dataloader(validation_input_directory, validation_files, args.sgd_batch_size, feat_dim,
                                    shuffle=False, last_batch=True):
                inputs, targets = batch
                if targets is None:
                    print('No Target given to minibatch iterator. Using a dummy target with two outputs')
                    targets = np.zeros((inputs.shape[0], 2))
            ...
            print("{} Set-Batch Validation done in {} seconds. Validation Loss {} ".format(i,
                                                                                           time.time() - start_time_val,
                                                                                           epo_val_loss))
        ...
        filename = os.path.join('output_' + str(args.run_number),
                                'metrics_validation_' + str(args.run_number) + '_' + str(epoch) + '_aps.txt')
        filename1 = os.path.join('output_' + str(args.run_number),
                                 'metrics_validation_' + str(args.run_number) + '_' + str(epoch) + '_aps_ranked.txt')
        filename2 = os.path.join('output_' + str(args.run_number),
                                 'metrics_validation_' + str(args.run_number) + '_' + str(epoch) + '_aucs.txt')
        ...
        if aps[-1] > aps_best:
            ...
            torch.save(net.state_dict(),
                       'output_' + str(args.run_number) + '/model_path.' + str(args.run_number) + '_' + str(
                           epoch) + '.pkl')
            model_path = 'output_' + str(args.run_number) + '/model_path.' + str(args.run_number) + '_' + str(
                epoch) + '.pkl'

        print("{} Validation done in {} seconds. Validation Loss {}".format(epoch, time.time() - start_time_val,
                                                                            epo_val_loss))
    ...
    for i in range(len(args.val_test_list)):
        ...
        if len(testing_files) == 0:
            continue
        for batch in dataloader(testing_input_directory, testing_files, args.sgd_batch_size, feat_dim, shuffle=False,
                                last_batch=True):
            inputs, targets = batch
            if targets is None:
                print('No Target given to minibatch iterator. Using a dummy target with two outputs')
        ...
        print("{} Set-Batch Testing done in {} seconds. Testing Loss {} ".format(i, time.time() - start_time_test,
                                                                                 epo_test_loss))
    ...
    filename = os.path.join('output_' + str(args.run_number),
                            'metrics_testing_' + str(args.run_number) + '_' + str(epoch_best) + '_aps.txt')
    filename1 = os.path.join('output_' + str(args.run_number),
                             'metrics_testing_' + str(args.run_number) + '_' + str(epoch_best) + '_aps_ranked.txt')
    filename2 = os.path.join('output_' + str(args.run_number),
                             'metrics_testing_' + str(args.run_number) + '_' + str(epoch_best) + '_aucs.txt')
    np.save(filename, aps)
    np.save(filename1, aps_ranked)
    np.save(filename2, aucs)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 13:
        ...
        setupClassifier(args.training_features_directory, args.validation_features_directory,
                        args.testing_features_directory, args.classCount, args.spec_count, args.segment_length,
                        args.learning_rate, args.output_path)
