...
def trim_audio(input_audio_file, output_audio_file, start_time, duration):
    ...
    cmdstring = "ffmpeg -loglevel panic -i %s -ss %s -t %s -c copy -y %s" % (
        input_audio_file, start_time, duration, output_audio_file)
    os.system(cmdstring)
...
def download_audio_method(line, csv_file):
    ...
    # Hacck to get the start time and end time
    start_seconds = line.split(",")[1].split(" ")[1];
    end_seconds = line.split(",")[2].split(" ")[1];
    ...
    try:
        ...
        path_to_download = output_folder + "/Y" + query_id + ".m4a"
        ...
        path_to_segmented_audio = segmented_folder + "/Y" + query_id + '_' + start_seconds + '_' + end_seconds + ".m4a"

        # print path_to_segmented_audio
        trim_audio(path_to_download, path_to_segmented_audio, start_seconds, audio_duration)
